...
router.put('/:contactId', async (req, res, next) => {
  try {
    ...
    const updatedContact = await updateContact(req.params.contactId, req.user._id, req.body);
...
router.patch('/:contactId/favorite', async (req, res, next) => {
  try {
    ...
    if (favorite === undefined) return res.status(400).json({message: 'missing field favorite'});

    const updatedContact = await updateStatusContact(req.params.contactId, req.user._id, favorite);
